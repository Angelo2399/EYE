...
from app.services.session_service import SessionContext, SessionPhase
...
def _build_service_with_fakes(
    ...
    *,
    ...
    regime: str = "bullish",
    volatility_20: float = 0.010,
    with_session_service: bool = True,
    ...
    if with_session_service:
        service.session_service = SimpleNamespace(
            get_session_context=lambda: session_context
        )
...
def test_generate_signal_returns_no_trade_when_session_is_closed() -> None:
    saved: dict[str, Any] = {}
    ...
    service = _build_service_with_fakes(
        ...
        setup_score_value=68.0,
        with_session_service=True,
...
def test_generate_signal_returns_no_trade_when_session_is_open_but_cutoff_is_too_close() -> None:
    saved: dict[str, Any] = {}
    session_context = SessionContext(
        phase=SessionPhase.power_hour,
        is_session_open=True,
        allow_new_trades=False,
        minutes_to_cutoff=10,
    )
    service = _build_service_with_fakes(
        saved,
        setup_action=SignalAction.long,
...
def test_generate_signal_keeps_working_without_session_service_attribute() -> None:
    saved: dict[str, Any] = {}
    service = _build_service_with_fakes(
        saved,
        setup_action=SignalAction.long,
        setup_score_value=68.0,
        with_session_service=False,
