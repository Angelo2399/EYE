...
class BEAConnector(BaseIntelligenceConnector):
    ...
    def __init__(
        self,
        *,
        page_url: str = "https://www.bea.gov/news/schedule",
        source_name: str = "U.S. Bureau of Economic Analysis",
    ...
    def fetch(
        ...
        try:
            ...
            result = ConnectorFetchResult(
                ...
                warnings=[] if items else ["No targeted BEA releases were found."],
                errors=[],
            )
            return items, result
    ...
    def _validate_request(self, request: ConnectorFetchRequest) -> None:
        if request.source_kind != ConnectorSourceKind.bea:
            raise ValueError("BEAConnector only supports source_kind='bea'.")

        if request.fetch_mode != ConnectorFetchMode.html:
            raise ValueError("BEAConnector only supports fetch_mode='html'.")
