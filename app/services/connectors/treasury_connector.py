...
class TreasuryConnector(BaseIntelligenceConnector):
    TARGET_RELEASES: tuple[tuple[str, str, IntelligenceImportance, list[str]], ...] = (
        (
            "auction announcement",
            "Auction Announcement",
            IntelligenceImportance.high,
            ["auctions", "funding", "rates"],
        ),
        (
            "refunding",
            "Refunding",
            IntelligenceImportance.critical,
            ["refunding", "funding", "liquidity", "rates"],
        ),
        (
            "borrowing estimate",
            "Borrowing Estimate",
            IntelligenceImportance.critical,
            ["borrowing", "funding", "liquidity"],
        ),
        (
            "debt management",
            "Debt Management Update",
            IntelligenceImportance.high,
            ["funding", "liquidity", "rates"],
        ),
        (
            "financing",
            "Financing Update",
    ...
    def __init__(
        self,
        *,
        page_url: str = "https://home.treasury.gov/news/press-releases",
        source_name: str = "U.S. Department of the Treasury",
    ...
    def fetch(
        ...
        try:
            ...
            result = ConnectorFetchResult(
                ...
                warnings=[] if items else ["No targeted Treasury releases were found."],
                errors=[],
            )
            return items, result
    ...
    def _validate_request(self, request: ConnectorFetchRequest) -> None:
        if request.source_kind != ConnectorSourceKind.treasury:
            raise ValueError("TreasuryConnector only supports source_kind='treasury'.")

        if request.fetch_mode != ConnectorFetchMode.html:
            raise ValueError("TreasuryConnector only supports fetch_mode='html'.")
