...
class _EIAListingParser(HTMLParser):
...
class EIAConnector(BaseIntelligenceConnector):
    def __init__(
        self,
        *,
        page_url: str = "https://www.eia.gov/petroleum/data.php",
        source_name: str = "U.S. Energy Information Administration",
    ...
    def fetch(
        ...
        try:
            ...
            result = ConnectorFetchResult(
                ...
                warnings=[] if items else ["No usable EIA items were found."],
                errors=[],
            )
            return items, result
    ...
    def _validate_request(self, request: ConnectorFetchRequest) -> None:
        if request.source_kind != ConnectorSourceKind.eia:
            raise ValueError("EIAConnector only supports source_kind='eia'.")

        if request.fetch_mode != ConnectorFetchMode.html:
            raise ValueError("EIAConnector only supports fetch_mode='html'.")
    ...
    def _parse_html(
        ...
        *,
        ...
        parser = _EIAListingParser(base_url=self._page_url)
    ...
    def _build_tags(
        ...
        *,
        ...
        tags: set[str] = {"eia", "oil", "energy"}

        text = f"{title} {url}".lower()
        keyword_tags = {
            "weekly petroleum status report": "wpsr",
            "weekly supply": "weekly_supply",
            "petroleum supply monthly": "petroleum_supply_monthly",
