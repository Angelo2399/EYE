...
class ECBConnector(BaseIntelligenceConnector):
    def __init__(
        self,
        *,
        feed_url: str = "https://www.ecb.europa.eu/rss/press.html",
        source_name: str = "European Central Bank",
    ...
    def _validate_request(self, request: ConnectorFetchRequest) -> None:
        if request.source_kind != ConnectorSourceKind.ecb:
            raise ValueError("ECBConnector only supports source_kind='ecb'.")

        if request.fetch_mode != ConnectorFetchMode.rss:
            raise ValueError("ECBConnector only supports fetch_mode='rss'.")
    ...
    def _build_tags(
        ...
        *,
        ...
        text = f"{title} {description}".lower()
        keyword_tags = {
            "governing council": "governing_council",
